...
public class Lokomotiv {
    ...
    private final int tepWidth = 140;
    private final int tepHeight = 100;
    ...
    public Lokomotiv(int maxSpeed, float weight, Color mainColor, Color dopColor) {
        MaxSpeed = maxSpeed;
        Weight = weight;
        MainColor = mainColor;
        DopColor = dopColor;
    ...
    public void MoveTransport(Direction direction) {
        ...
        switch (direction) {
            case Left:
                ...
                break;
            case Right:
                if (_startPosX + step + tepWidth < _pictureWidth) { _startPosX += step; }
                break;
            case Up:
                if (_startPosY - step > 0) { _startPosY -= step; }
                break;
            case Down:
                if (_startPosY + step + tepHeight < _pictureHeight) { _startPosY += step; }
                break;
        }
    }

    public void DrawLokomotiv(Graphics g) {
